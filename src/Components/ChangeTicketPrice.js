...
export const ChangeTicketPrice = ({ onClose, onSubmit }) => {
  ...
  const fetchdata = async () => {
    try {
      if (window.ethereum) {
        ...
        setcurrentprice(ethers.utils.formatEther(data.toString()));
      } else {
    ...
    if (address === lotterywalletaddress) {
      try {
        if (window.ethereum) {
          ...
          const tx = await contract.changeticketprice(
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-6 flex justify-center items-center">
            <label
              ...
              htmlFor="name"
            >
              <span className="text-blue-800 font-bold">Old Price:</span>{" "}
              {currentprice}{" "}
