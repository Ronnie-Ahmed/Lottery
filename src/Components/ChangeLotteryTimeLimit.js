...
export const ChangeLotteryTimeLimit = ({ onClose, onSubmit }) => {
  ...
  const fetchdata = async () => {
    try {
      if (window.ethereum) {
        ...
        const data = await contract.getLotterytime();
    ...
    if (address === lotterywalletaddress) {
      try {
        if (window.ethereum) {
          ...
          const tx = await contract.changeLotterytime(newPrice);
          await tx.wait();
          ...
          <div className="mb-6 flex justify-center items-center">
            ...
            >
              <span className="text-blue-800 font-bold">
                Old Lottery Time Limit:
              </span>{" "}
              {currentprice}{" "}
              <span className="text-blue-800 font-bold">Blocks</span>
            </label>
          </div>
          <div className="mb-6">
            <label
              ...
              htmlFor="name"
            >
              New Geting Tickets Time Limits
            </label>
            <input
